...
export function CreateUpdateAccommodation(props: CreateUpdateAccommodationProps) {
  ...
  const onSubmitImages = (data: string) => {
    images.push(data);
    setImages(images);
    resetImages();
  };
  ...
  const onSubmit = (data: any) => {
    CreateUpdateAccommodationFunction(data, amenities, images, localStorage.getItem('userId')!);
        ...
        <div className={styles.lineContainer}>
          ...
          <div className={styles.lineContainer} style={{ gap: '1rem' }}>
            ...
            <input
              style={{ width: '48px', height: '48px' }}
              type="checkbox"
              id="hasAutomaticReservations"
              {...register('hasAutomaticReservations', {
                required: 'This field is required.',
              })}
            />
          </div>
