...
func (store *UserRepository) GetByApiKey(apiKeyValue string) (bool, string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	usersCollection := store.getCollection()

	filter := bson.M{"apikey.value": apiKeyValue}
	...
	if !user.APIKey.IsValid() {
		return false, user.ID.Hex(), nil
	}
